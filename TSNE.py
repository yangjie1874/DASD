...
class T_sne_visual():
    ...
    def visual_feature_map(self, layer):
        ...
        with torch.no_grad():
            ...
            self.t_sne(np.array(self.feature_map_list.cpu()), np.array(labels),title=f'visualize result\n')
...
trans=transforms.Compose([transforms.Resize((320,320)),transforms.ToTensor(),transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
imgset=torchvision.datasets.ImageFolder(root='/media/bao511/新加卷/4',transform=trans)
img_loader=DataLoader(imgset,batch_size=3,shuffle=True)
