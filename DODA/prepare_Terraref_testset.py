...
#generate iamges according to the bounding boxes and reference iamges of the terraref domain:
#Path of Global Wheat Head Detection Datas,
#gwhd_2021_path = 'datasets/gwhd_2021/'
gwhd_2021_path = 'datasets/scab2021/'



#Path to save the images of Terraref domain
Terraref_path = 'datasets/scab2021/Terraref1/target/'
makedir(Terraref_path)
Terraref_ori_source_path = 'datasets/scab2021/Terraref1/source/'
makedir(Terraref_ori_source_path)
Terraref_cut_Path = 'datasets/scab2021/Terraref_x91/'
...
for imgName in tqdm(os.listdir(Terraref_path)):
    ...
    if imgName in label_dic.keys():
        ...
        Terraref_label.write('%s.png' % imgName)
        if 'no_box' not in laBel:
            orginal_bbox = np.array([np.array(list(map(int, bbox.split(',')))) for bbox in laBel])

            bboxes = orginal_bbox[np.logical_and((orginal_bbox[:, 2] - orginal_bbox[:, 0]) > 10,
                                                 (orginal_bbox[:, 3] - orginal_bbox[:, 1]) > 10)]  # discard invalid box
            if bboxes.shape[0] > 0:
                names_img_with_wheat.write('%s.png' % imgName + '\n')
                for bbox in bboxes:
                    bbox = ','.join(map(str, bbox))
                    Terraref_label.write(' ' + bbox + ',0')
            else:
                names_img_wo_wheat.write('%s.png' % imgName + '\n')
        else:
            names_img_wo_wheat.write('%s.png' % imgName + '\n')
