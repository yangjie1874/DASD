...
def process(control_image, reference_image, num_samples, image_resolution, ddim_steps, guess_mode, strength, scale,
            seed, eta):
    with torch.no_grad():
        if seed == -1:
        ...
        un_cond = {"c_concat": None if guess_mode else [control],
                   "c_crossattn": [model.get_learned_conditioning(torch.zeros((1, 3, 224, 224)).cuda())]}
        shape = (3, H // 4, W // 4)
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            control_image = gr.Image(sources='upload', type="numpy")
            reference_image = gr.Image(sources='upload', type="numpy")
    ...
    ips = [control_image, reference_image, num_samples, image_resolution, ddim_steps, guess_mode, strength, scale, seed,
           eta]
